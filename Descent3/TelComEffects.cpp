...
void RenderEffect(tceffect* tce, float frametime, int xoff, int yoff, bool ok_to_render)
{
	...
	switch (tce->type)
	{
	case EFX_TEXT_STATIC:		RenderTextStatic(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_TEXT_TYPE:			RenderTextType(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_TEXT_FADE:			RenderTextFade(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_BMP_STATIC:		RenderBmpStatic(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_BMP_BLUR:			RenderBmpBlur(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_BMP_SCANLINE:		RenderBmpScanline(tce, frametime, xoff, yoff, ok_to_render); break;
	case EFX_BMP_INVERT:		RenderBmpInvert(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_BMP_STRETCH:		RenderBmpStretch(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_BACKGROUND:		RenderBackground(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_MOVIE:				RenderMovie(tce, frametime, xoff, yoff, ok_to_render);		break;
	case EFX_POLYMODEL:			RenderPolyModel(tce, frametime, xoff, yoff, ok_to_render);	break;
	case EFX_SOUND:				RenderSound(tce, frametime, xoff, yoff, ok_to_render);		break;
	case EFX_BUTTON:			RenderButton(tce, frametime, xoff, yoff, ok_to_render);		break;
	default:
		Int3();
	};
...
int FindButtonEffectByXY(int x, int y, int screen)
{
	...
	while (node != -1)
	{
		...
		{
			tce = &TCEffects[node];
			if ((tce->pos_x + glitch_dx <= x) && (x <= tce->pos_x + tce->w + glitch_dx) &&
				(tce->pos_y + glitch_dy <= y) && (y <= tce->pos_y + tce->h + glitch_dy))
			{
				//we got a winner
				return node;
			}
...
bool CreateTextEffect(LPTCTEXTDESC desc, char* text, int monitor, int screen, int id)
{
	...
	int efxtype = -1;
	switch (desc->type) {
	case TC_TEXT_STATIC:		efxtype = EFX_TEXT_STATIC;		break;
	case TC_TEXT_FADE:			efxtype = EFX_TEXT_FADE;		break;
	case TC_TEXT_FLASH:			efxtype = EFX_TEXT_TYPE;		break;
	default:
		Int3();	//not supported
	}
	if (efxtype == -1)
	...
	switch (tce->type)
	{
	case EFX_TEXT_STATIC:	return CreateTextStatic(tce, text);
	case EFX_TEXT_FADE:		return CreateTextFade(tce, text);
	case EFX_TEXT_TYPE:		return CreateTextType(tce, text);
	}
	return false;
}
...
bool CreateBitmapEffect(LPTCBMPDESC desc, int monitor, int screen, int id)
{
	...
	switch (desc->type)
	{
	case TC_BMP_STATIC:		efxtype = EFX_BMP_STATIC;	break;
	case TC_BMP_BLUR:		efxtype = EFX_BMP_BLUR;		break;
	case TC_BMP_SCANLINE:	efxtype = EFX_BMP_SCANLINE;	break;
	case TC_BMP_INVERT:		efxtype = EFX_BMP_INVERT;	break;
	case TC_BMP_STRETCH:	efxtype = EFX_BMP_STRETCH;	break;
	default:
		Int3();
	}
	if (efxtype == -1)
	...
	switch (tce->type)
	{
	case EFX_BMP_STATIC:		return CreateBmpStatic(tce, desc->filename);
	case EFX_BMP_BLUR:			return CreateBmpBlur(tce, desc->filename);
	case EFX_BMP_SCANLINE:		return CreateBmpScanline(tce, desc->filename);
	case EFX_BMP_INVERT:		return CreateBmpInvert(tce, desc->filename);
	case EFX_BMP_STRETCH:		return CreateBmpStretch(tce, desc->filename);
	}
	return false;
}
...
bool CreateMovieEffect(LPTCMOVIEDESC desc, int monitor, int screen, int id)
{
	...
	switch (desc->type)
	{
	case TC_MOVIE_STATIC:		efxtype = EFX_MOVIE;	break;
	default:
		Int3();
	}
	if (efxtype == -1)
	...
	switch (tce->type)
	{
	case EFX_MOVIE:		return CreateMovie(tce, desc->filename);
	}
...
bool CreateBackgroundEffect(LPTCBKGDESC desc, int monitor, int screen, int id)
{
	...
	switch (tce->type)
	{
	case EFX_BACKGROUND:		return CreateBackground(tce);
	}
	return false;
}
...
bool CreatePolyModelEffect(LPTCPOLYDESC desc, int monitor, int screen, int id)
{
	...
	switch (desc->type)
	{
	case TC_POLY_STATIC:	efxtype = EFX_POLYMODEL;	break;
	default:
		Int3();
	}
...
bool CreateSoundEffect(LPTCSNDDESC desc, int monitor, int screen, int id)
{
	...
	switch (desc->type)
	{
	case TC_SND_STATIC:		efxtype = EFX_SOUND;	break;
	default:
		Int3();
	}
	if (efxtype == -1)
	...
	switch (tce->type)
	{
	case EFX_SOUND:			return CreateSound(tce, desc->filename);
	}
	return false;
}
