...
void osipf_ObjectCustomAnim(int handle, float start, float end, float time, char flags, int sound_handle, char next_anim_type)
{
...
{
	return FindObjectIDName(IGNORE_TABLE(name));
}
...
{
	return FindWeaponName(IGNORE_TABLE(name));
}
...
int osipf_AISetGoalFlags(int objhandle, int goal_handle, int flags, ubyte f_enable)
{
	...
	if (goal_handle >= 0 &&
		goal_handle < MAX_GOALS &&
		ai_info->goals[goal_handle].used != 0)
	{
		if (f_enable)
...
{
	return cf_ReadBytes(buffer, count, cfp);
}
...
{
	return cf_ReadString(buf, n, cfp);
}
...
{
	return cf_WriteBytes(buf, count, cfp);
}
		...
		if (!(Game_mode & GM_MULTI))
		{
			if (obj->handle == Buddy_handle[0])
			{
				if (stricmp(Ships[Players[Player_object->id].ship_index].name, "Black Pyro") == 0)
				{
					...
					PageInPolymodel(Object_info[ROBOT_GUIDEBOTRED].render_handle, Object_info[ROBOT_GUIDEBOTRED].type, &Object_info[ROBOT_GUIDEBOTRED].size);
...
{
	return FindSoundName(IGNORE_TABLE(s_name));
}
...
int osipf_AIGetNearbyObjs(vector* pos, int init_roomnum, float rad, int* object_handle_list, int max_elements, bool f_lightmap_only, bool f_only_players_and_ais, bool f_include_non_collide_objects, bool f_stop_at_closed_doors)
{
	...
	num_close = fvi_QuickDistObjectList(pos, init_roomnum, rad, s_list, max_elements, f_lightmap_only, f_only_players_and_ais, f_include_non_collide_objects, f_stop_at_closed_doors);
