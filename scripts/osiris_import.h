...
inline void Obj_GetGunPos(int objhandle,int gun_number,vector *gun_pnt,vector *gun_normal = NULL)
{
...
inline void Obj_GetGroundPos(int objhandle,int ground_number,vector *ground_pnt,vector *ground_normal = NULL)
{
...
inline int AI_GoalFollowPathSimple(int objhandle,int path_id,int guid,int flags,int slot = 3)
{
...
inline void Obj_Value(int objhandle, char op, char vtype, void *ptr, int index=0)
{
...
inline void Matcen_Value(int matcen_handle, char op, char vtype, void *ptr, int prod_index = 0)
{
...
inline int AI_FindObjOfType(int objhandle,int type, int id, bool f_ignore_init_room, int parent_handle = OBJECT_HANDLE_NONE)
{
...
typedef int( *Obj_Create_fp)(ubyte type,ushort id,int roomnum,vector *pos,const matrix *orient ,int parent_handle,vector *initial_velocity);
OSIRISEXTERN Obj_Create_fp Obj_CreateFP;
inline int Obj_Create(ubyte type,ushort id,int roomnum,vector *pos,const matrix *orient=NULL,int parent_handle=NULL, vector *initial_velocity=NULL)
{
...
typedef void(*Obj_SetCustomAnim_fp)(int handle, float start, float end, float time, char flags, int sound_handle, char next_anim_type);
...
inline void Sound_Stop(int s_handle, bool f_immediately=true)
{
  Sound_StopFP(s_handle,f_immediately);
}
...
typedef int (*AI_GetNearbyObjs_fp)(vector *pos, int init_roomnum, float rad, int *object_handle_list, int max_elements, bool f_lightmap_only, bool f_only_players_and_ais, bool f_include_non_collide_objects, bool f_stop_at_closed_doors);
...
{
  return AI_GetNearbyObjsFP(pos,init_roomnum,rad,object_handle_list,max_elements,f_lightmap_only,f_only_players_and_ais,f_include_non_collide_objects,f_stop_at_closed_doors);
}
...
typedef void (*Game_CreateRandomSparks_fp)(int num_sparks,vector *pos,int roomnum,int which_index,float force_scalar);
...
{
	int i = 0;
#define OSIRIS_IMPORT_ASSIGN_GUEST_SLOT(guest_symbol, guest_type, host_symbol, abi_argc, test_argc, return_kind, bridge_kind) \
	guest_symbol = (guest_type##_fp)mi->fp[i++];
	OSIRIS_IMPORT_LIST(OSIRIS_IMPORT_ASSIGN_GUEST_SLOT)
#undef OSIRIS_IMPORT_ASSIGN_GUEST_SLOT
}
#endif
