...
inline int WRITE_FONT_INT(FONTFILE2 ffile, int i) 
{
	return fwrite(&i, sizeof(i), 1, (FILE *)ffile);
}

inline int WRITE_FONT_SHORT(FONTFILE2 ffile, short s) 
{
	return fwrite(&s, sizeof(s), 1, (FILE *)ffile);
}

inline int WRITE_FONT_BYTE(FONTFILE2 ffile, ubyte c) 
{
...
{
	int i = (int)fwrite(buf, size, nelem, (FILE *)ffile);
...
{
	FONTFILE2 fp = (FONTFILE2)fopen(filename, "wb");
	if (!fp) return NULL;

	return fp;
}

inline void CLOSE_FONT2(FONTFILE2 ffile) 
{
	fclose((FILE *)ffile);
}
...
{
	if (Font_init) 
		grfont_Close(); //free all allocated fonts.
	else 
		atexit(grfont_Close);
...
//	delete font surface info.
	tFontInfo* ft = &Fonts[handle];
	if (ft->ch_bmp) delete[] ft->ch_bmp;
	...
	for (int i = 0; i < MAX_FONT_BITMAPS; i++)
	{
		if (ft->bmps[i] != -1) 
...
bool grfont_LoadTemplate(char *fname, tFontTemplate *ft)
{
	...
	//read header information
	short ft_width = READ_FONT_SHORT(ff);
	short ft_height = READ_FONT_SHORT(ff);
	short ft_flags = READ_FONT_SHORT(ff);
	READ_FONT_SHORT(ff);									// skip baseline value (HACKED FOR BRIGHTNESS)
	short ft_minasc = READ_FONT_BYTE(ff);
	short ft_maxasc = READ_FONT_BYTE(ff);
...
bool grfont_SetTemplate(const char *pathname, const tFontTemplate *ft)
{
	...
	int num_char = fnt.max_ascii-fnt.min_ascii+1;
	...
	fnt.flags = (ft->proportional ? FT_PROPORTIONAL : 0) + ((!ft->monochromatic) ? FT_COLOR : 0) + (ft->kern_data ? FT_KERNED : 0)
						+ (ft->newstyle ? FT_FMT4444 : 0);
	fnt.min_ascii = (ubyte)ft->min_ascii;
	fnt.max_ascii = (ubyte)ft->max_ascii;

	if (ft->ffi2) fnt.flags |= FT_FFI2;
	...
	//write out font.
	FONTFILE2 ffout = OPEN_FONT2(pathname);
	if (!ffout)
		return false;
...
//	save current x and get this font
	...
	if (!cbi->clipped) 
	{
		...
		rend_DrawFontCharacter (ft->bmps[ft->ch_bmp[cbi->ch]], cbi->x,cbi->y,
											(int)(cbi->x+cbi->sw*cbi->dsw),
											(int)(cbi->y+cbi->sh*cbi->dsh),
											ft->ch_uf[cbi->ch], 
											ft->ch_vf[cbi->ch], 
											ft->ch_wf[cbi->ch], 
											ft->ch_hf[cbi->ch]);
	...
	else 
	{
		rend_DrawFontCharacter (ft->bmps[ft->ch_bmp[cbi->ch]], cbi->x,cbi->y,
											...
											(int)(cbi->y+cbi->sh),
											//[ISB] need to divide out the scale which shouldn't apply to UVs why is the grfont code some of the worst text rendering my eyes have seen. 
											ft->ch_uf[cbi->ch]+(((float)cbi->sx / cbi->dsw)/((float)GRFONT_SURFACE_WIDTH)), 
											ft->ch_vf[cbi->ch]+(((float)cbi->sy / cbi->dsh)/((float)GRFONT_SURFACE_HEIGHT)),
											((float)cbi->sw / cbi->dsw)/((float)GRFONT_SURFACE_WIDTH),
											((float)cbi->sh / cbi->dsh)/((float)GRFONT_SURFACE_HEIGHT));
...
int grfont_GetCharWidth(int font, int ch)
{
	...
	tFontFileInfo* ft = &Fonts[font].font;
	ch &= 255;
...
void grfont_TranslateToBitmaps(int handle)
{
	...
	tFontInfo* fnt = &Fonts[handle];
	tFontFileInfo* fntfile = &Fonts[handle].font;
	...
	int num_ch = fntfile->max_ascii-fntfile->min_ascii+1;
	...
	for (int ch = 0; ch < num_ch; ch++)
	{
		...
		if ((u+w) > GRFONT_SURFACE_WIDTH) 
		{
			...
			if ((v+fntfile->height) > GRFONT_SURFACE_HEIGHT) 
			{
				if (surf_index == MAX_FONT_BITMAPS)
					Error("grfont_TranslateToBitmaps: Font bitmap limit exceeded!");
				fnt->bmps[surf_index] = bm_AllocBitmap(GRFONT_SURFACE_WIDTH, GRFONT_SURFACE_HEIGHT, 0);																
				if (fnt->bmps[surf_index] == -1 || fnt->bmps[surf_index] == BAD_BITMAP_HANDLE) 
					Error("TranslateToBitmaps <Bitmap allocation error>");
				if (fntfile->flags & FT_FMT4444)
		...
		if (fntfile->flags & FT_COLOR) 
		{
			...
			//[ISB] I have no idea why the else on this was commented out.
...
ushort *grfont_GetRawCharacterData(int font, int ch, int *w, int *h, bool *mono)
{
	...
	tFontInfo* fnt = &Fonts[font];
	tFontFileInfo* fntfile = &fnt->font;
